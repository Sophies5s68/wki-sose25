...
# ---- Datei zum Trainierne des Modells ohne StratifiedGroupKfold ----
...
def main():
    ...
    # Splitter zum Aufteilen des Datensatzes anhand der PatientenIDs
    ...
    # Threshold-Tuning auf val_loader
    all_probs = []
    all_targets = []
    with torch.no_grad():
    ...
    with open(os.path.join(save_path, "threshold_results.txt"), "w") as f:
        f.write(f"Best Threshold: {best_thresh:.4f}\n")
        f.write(f"F1 Score:       {best_f1:.4f}\n")
        f.write(f"Precision:      {best_precision:.4f}\n")
        f.write(f"Recall:         {best_recall:.4f}\n")

    with open(os.path.join(save_path, "threshold_metrics.csv"), "w", newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Threshold", "F1", "Precision", "Recall"])
        for row in metrics_list:
            writer.writerow([f"{row[0]:.4f}", f"{row[1]:.4f}", f"{row[2]:.4f}", f"{row[3]:.4f}"])

    np.save(os.path.join(save_path, "val_probs.npy"), all_probs)
    np.save(os.path.join(save_path, "val_labels.npy"), all_targets)
