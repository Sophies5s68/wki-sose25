...
# === Anfallserkennung + Onset-Vorhersage-Pipeline ===
...
batch_size = 16
num_epochs = 50
step_size = 2  # Sekunden

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Training auf {device}")

# === Patienten-Dateien und Labels laden ===
patient_to_path = {}
patient_to_label = {}
...
for i, fname in enumerate(file_list):
    ...
    print(f"Geladen {i+1}/{len(file_list)}: {fname}", end='\r')

print(f"\nInsgesamt {len(patient_to_path)} Patientendateien geladen.")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(patient_ids, patient_labels)):
    ...
    print(f"\nFold {fold+1} | Training: {len(train_ids)} | Test: {len(test_ids)}")
    break
num_seizure_patients = sum([1 for pid in train_ids if patient_to_label[pid] == 1])
print("Anfallspatienten im Trainingsset:", num_seizure_patients)
...
class EEGSequenceDataset(Dataset):
    def __init__(self, ids, paths, labels=None, onset_mode=False, chunk_len=10, stride=10, cache_data=False):
        ...
        self.chunk_labels = []  # Nur wenn nicht im Onset-Modus
...
print("Trainings- und Test-Datensätze erstellt")
...
print("Trainings- und Test-Daten geladen")

counter = Counter(train_dataset_det.chunk_labels)
print("Trainingsklassendistanz (Chunks):", counter)

sample_input, _ = next(iter(train_loader_det))

# === Stage 1: Anfallserkennungsmodell ===
...
print("\n=== Stufe 1: Training der Anfallserkennung ===")
for epoch in range(1, num_epochs + 1):
    ...
    print(f"Epoche {epoch} | F1: {f1:.4f} | Genauigkeit: {acc:.4f}")
    print("Vorhergesagte Klassenverteilung:", Counter(all_preds))
    print("Wahre Klassenverteilung:", Counter(all_targets))

# === Stage 2: Onset-Vorhersage (nur bei Anfallpatienten) ===
print("\n=== Stufe 2: Training der Onset-Vorhersage ===")
...
for epoch in range(1, num_epochs + 1):
    ...
    f1 = f1_score(all_targets, all_preds)
    acc = accuracy_score(all_targets, all_preds)
    print(f"Epoche {epoch} | Onset F1: {f1:.4f} | Onset Genauigkeit: {acc:.4f}")
    if onset_errors:
        print(f"Durchschnittlicher Onset-Fehler: {np.mean(onset_errors):.2f} Sekunden")
