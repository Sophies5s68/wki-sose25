...
for fold, (train_idx, test_idx) in enumerate(skf.split(patient_ids, labels_array)):
    ...
    weights = [1.0 / label_counts[label] for label in train_dataset.chunk_labels]
    sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)

    train_loader = DataLoader(train_dataset, batch_size=batch_size, sampler=sampler, collate_fn=collate_fn_detection)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_detection)
