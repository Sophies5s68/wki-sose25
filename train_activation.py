...
from CNN_activations import CNN_EEG, train_model, evaluate_model  # Stelle sicher, dass du die Klasse separat speicherst
import numpy as np
...
# Datei zum automatisieren Trainieren von dem Modell mit verschiedenen Aktivierungsfunktionen
...
def main():
    ...
    for name, act_fn in activation_functions.items():
        ...
        for i, (train_idx, val_idx) in enumerate(sgkf.split(X=range(len(dataset)), y=labels, groups=groups)):
            ...
            model = CNN_EEG(in_channels=dataset[0][0].shape[0], n_classes=1, activation_fn = act_fn).to(device)
